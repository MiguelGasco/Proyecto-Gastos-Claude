...
# Cross-project: trades + watchlist swing (Proyecto Finanzas Claude)
...
def render(
    ...
    watchlist_swing_path: Path = DEFAULT_WATCHLIST_SWING_PATH,
) -> Path:
    ...
    if trades_path.exists():
        try:
            ...
            print(f"  (warn: no pude leer trades.json: {e})")

    watchlist_swing = {}
    if watchlist_swing_path.exists():
        try:
            with open(watchlist_swing_path, encoding="utf-8") as f:
                watchlist_swing = json.load(f)
        except Exception as e:
            print(f"  (warn: no pude leer watchlist_swing.json: {e})")
    ...
    watchlist_swing_json = json.dumps(watchlist_swing, ensure_ascii=False)
    ...
    html = (
        ...
        .replace("__WATCHLIST_SWING_JSON__", watchlist_swing_json)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Genera investments.html desde data/investments.json + data/quotes.json + cartera/trades.json + cartera/watchlist_swing.json y el template."
    )
    ...
    parser.add_argument("--watchlist-swing", default=str(DEFAULT_WATCHLIST_SWING_PATH))
